...
async def logUserBot():
    load_dotenv()
    api_id = int(29205034)
    api_hash = "aedb2d993c160d60c1ac5d6d81003fa0"
    phone_number = "51988041614"
    ...
    await client.send_message("@spmmatias", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4687615477")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmmatias", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam"] and i['group_id'] not in[-1002351366917,-1002331326186,-1002425258747,-1002432211914,-1002216176545,-1001926519077,-1001580673964,-1001907073788,-1001737351681,-1001733869168,-1001829996546,-1002018025154,-1001859082953,-4553972318,-4553791708,-4559750226,-4586567990,-1001724620371,-1001760634472,-1002125807620,-1001617010310,-1001867739320,-1002074331354,-1001789640951,-1001899823705,-1001890531963,-1002296434091]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmmatias", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmmatias", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        else: break
                        await asyncio.sleep(90)
                        if j==1: break
            await client.send_message("@spmmatias", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
